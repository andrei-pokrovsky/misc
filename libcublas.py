...
def _get_cublas_version():
    ...
    cublas_path = so_utils.find_lib_path('cublas')
    try:
        major, minor = re.search('[\D\.]+\.+(\d+)\.(\d+)',
                                 so_utils.get_soname(cublas_path)).groups()
    except:
...
_libcublas.cublasHgemm.restype = int
_libcublas.cublasHgemm.argtypes = [_types.handle,
                                      ctypes.c_int,
                                      ctypes.c_int,
                                      ctypes.c_int,
                                      ctypes.c_int,
...
def cublasHgemm(handle, transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, C, ldc):
    """
    Matrix-matrix product for real half precision (fp16) general matrix.
    ...
    status = _libcublas.cublasHgemm(handle,
                                       _CUBLAS_OP[transa],
                                       _CUBLAS_OP[transb], m, n, k,
                                       ctypes.byref(ctypes.c_short(alpha16)),
                                       int(A), lda, int(B), ldb,
                                       ctypes.byref(ctypes.c_short(beta16)),
                                       int(C), ldc)
    cublasCheckStatus(status)
